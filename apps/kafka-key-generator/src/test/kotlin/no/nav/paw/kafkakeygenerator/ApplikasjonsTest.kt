...
class ApplikasjonsTest : StringSpec({
    ...
    val pdlKlient = PdlClient(
        url = "http://mock", tema = "tema", HttpClient(MockEngine {
            genererResponse(it)
        })
    ) { "fake token" }
    val app = Applikasjon(
        kafkaKeys = KafkaKeys(Database.connect(dataSource)), identitetsTjeneste = PdlIdentitesTjeneste(pdlKlient)
    )

    fun hentEllerOpprett(identitetsnummer: String): Either<Failure, ArbeidssoekerId> = runBlocking {
        app.hentEllerOpprett(
            callId = CallId(UUID.randomUUID().toString()), identitet = Identitetsnummer(identitetsnummer)
        )
    }
    "alle identer for person1 skal gi samme nøkkel" {
        val person1KafkaNøkler = listOf(
            person1_dnummer, person1_fødselsnummer, person1_aktor_id, person1_annen_ident, person1_dnummer
        ).map(::hentEllerOpprett)
        person1KafkaNøkler.filterIsInstance<Left<Failure>>().size shouldBe 0
        person1KafkaNøkler.filterIsInstance<Right<ArbeidssoekerId>>().map { it.right }.distinct().size shouldBe 1
        app.hentLokaleAlias(2, listOf(person1_dnummer)).onLeft { fail { "Uventet feil: $it" } }.onRight { res ->
    ...
    "alle identer for person2 skal gi samme nøkkel" {
        val person2KafkaNøkler = listOf(
            person2_fødselsnummer, person2_aktor_id, person2_fødselsnummer
        ).map(::hentEllerOpprett)
        person2KafkaNøkler.filterIsInstance<Left<Failure>>().size shouldBe 0
        person2KafkaNøkler.filterIsInstance<Right<ArbeidssoekerId>>().map { it.right }.distinct().size shouldBe 1
    }
