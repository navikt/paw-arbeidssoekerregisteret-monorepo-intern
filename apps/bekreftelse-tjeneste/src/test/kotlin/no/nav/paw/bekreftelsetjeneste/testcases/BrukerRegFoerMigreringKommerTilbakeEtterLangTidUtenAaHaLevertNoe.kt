...
class BrukerRegFoerMigreringKommerTilbakeEtterLangTidUtenAaHaLevertNoe : FreeSpec({
    ...
    with(
        ...
        val stoppTid = "24.12.2025 00:00".timestamp
        val (hendelser, input) = with(KafkaKeyContext(this.kafkaKeysClient)) {
            ...
            val eksterneHendelser: List<Pair<Instant, ValueWithKafkaKeyData<*>>> = listOf(
                ...
                "24.11.2025 12:30".timestamp to ValueWithKafkaKeyData(periode.id, periode.key, stoppPaaVegneAv(
                    periodeId = periode.value.id,
                    bekreftelsesloesning = no.nav.paw.bekreftelse.paavegneav.v1.vo.Bekreftelsesloesning.DAGPENGER
                ))
            )
        ...
        forventer<BekreftelseTilgjengelig>(
            kilde,
            inputHendelser,
            fra = "28.11.2025 00:00".timestamp,
            til = "28.11.2025 23:00".timestamp,
            asserts = { publiserteBekreftelser ->
                publiserteBekreftelser.size shouldBe 1
                publiserteBekreftelser.first() should { bekreftelse ->
                    bekreftelse.gjelderFra shouldBe "17.11.2025 00:00".timestamp
                    bekreftelse.gjelderTil shouldBe "01.12.2025 00:00".timestamp
                }
        ...
        forventer<LeveringsfristUtloept>(
            kilde,
            inputHendelser,
            fra = "01.12.2025 00:00".timestamp,
            til = "01.12.2025 00:10".timestamp,
            asserts = { fristUtloeptListe ->
                fristUtloeptListe.size shouldBe 1
                fristUtloeptListe.first() should { fristUtloept ->
                    fristUtloept.leveringsfrist shouldBe "01.12.2025 00:00".timestamp
